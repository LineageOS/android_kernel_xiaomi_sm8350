...
static int aw8624_haptic_set_repeat_wav_seq(struct aw8624 *aw8624,
					    unsigned char seq)
{
	aw_dev_info(aw8624->dev, "%s repeat wav seq %d\n", __func__, seq);
 ...
 *
 *****************************************************/
int aw8624_parse_dt(struct device *dev, struct aw8624 *aw8624,
			   struct device_node *np)
{
	...
	val = of_property_read_u32(np, "aw8624_vib_mode", &aw8624->info.mode);
	if (val != 0)
		pr_err("%s: vib_mode not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_f0_pre", &aw8624->info.f0_pre);
	if (val != 0)
	...
	val = of_property_read_u32(np, "aw8624_vib_cont_td", &aw8624->info.cont_td);
	if (val != 0)
	...
	val = of_property_read_u32(np, "aw8624_vib_f0_coeff", &aw8624->info.f0_coeff);
	if (val != 0)
		pr_err("%s: vib_f0_coeff not found\n", __func__);

	val = of_property_read_u32(np, "aw8624_vib_tset", &aw8624->info.tset);
	if (val != 0)
		pr_err("%s: vib_tset not found\n", __func__);

	val = of_property_read_u32(np, "aw8624_vib_r_spare", &aw8624->info.r_spare);
	if (val != 0)
	...
	val =
	    of_property_read_u32_array(np, "aw8624_vib_bemf_config", bemf_config,
	...
	val =
	    of_property_read_u32(np, "aw8624_vib_gain_flag", &aw8624->info.gain_flag);
	if (val != 0)
...
int aw8624_haptics_upload_effect(struct input_dev *dev,
...
int aw8624_haptics_playback(struct input_dev *dev, int effect_id,
			    int val)
{
	struct aw8624 *aw8624 = input_get_drvdata(dev);
	int rc = 0;
...
int aw8624_haptics_erase(struct input_dev *dev, int effect_id)
{
	struct aw8624 *aw8624 = input_get_drvdata(dev);
	int rc = 0;
	/*for osc calibration */
...
void aw8624_haptics_set_gain_work_routine(struct work_struct *work)
{
...
void aw8624_haptics_set_gain(struct input_dev *dev, u16 gain)
{
	struct aw8624 *aw8624 = input_get_drvdata(dev);
	pr_debug("%s: enter\n", __func__);
			      ...
			      char *buf)
{
	...
	aw8624_haptic_upload_lra(aw8624, NORMAL_CALI);
...
int aw8624_vibrator_init(struct aw8624 *aw8624)
{
	...
	ret = sysfs_create_group(&aw8624->i2c->dev.kobj,
